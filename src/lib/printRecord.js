...
function buildArticleRows(article, record) {
  ...
  const checkedSensory = sensory.filter((i) => i.checked);
  const checkedPhysical = physical.filter((i) => i.checked);
  const checkedLabels = labels.filter((i) => i.checked);
  const maxRows = Math.max(checkedSensory.length, checkedPhysical.length, checkedLabels.length, 1);
  let rows = "";
  for (let r = 0; r < maxRows; r++) {
    const s = checkedSensory[r];
    const p = checkedPhysical[r];
    const l = checkedLabels[r];
    rows += "<tr>";
    if (r === 0) {
    ...
    const lRemark = l?.remark || "";
    rows += `<td>${l ? "&#10003; " + esc(getLabel(LABEL_CHECK_PARAMS, l.parameter)) + (lRemark && lRemark !== "OK" ? "<br>Remark: " + esc(lRemark) : "") : ""}</td>`;
    if (r === 0) {
