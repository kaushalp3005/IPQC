...
import { getSession, getCredentials } from "@/lib/auth";
...
function ViewContent() {
  ...
  async function handleUpdate(data) {
    ...
    setSaving(true);
    try {
      const res = await ipqc.update(ipqcNo, data, creds);
      setRecord(res);
  ...
  async function handleApprove() {
    ...
    setApproving(true);
    try {
      const res = await ipqc.approve(ipqcNo, creds);
      setRecord(res);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      ...
      {isAdmin ? (
        <IPQCForm initialData={record} onSubmit={handleUpdate} loading={saving} />
      ) : (
        <IPQCForm initialData={record} onSubmit={() => {}} loading={false} readOnly />
      )}
    </div>
  );
}
